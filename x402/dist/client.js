...
import { X402_PAYMENT_REQUIRED_HEADER, X402_PAYMENT_HEADER, } from './types.js';
...
export class X402Client {
     ...
     */
    selectAcceptOption(paymentRequired) {
        // Prefer exact match on our network
        const match = paymentRequired.accepts.find((a) => a.network === this.network && a.scheme === 'exact');
        if (match)
            return match;
        // Fall back to any Solana network with exact scheme
        return (paymentRequired.accepts.find((a) => a.network.startsWith('solana') && a.scheme === 'exact') ?? null);
    }
