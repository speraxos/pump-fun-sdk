...
In LairOS, file types, usually identified from the extension stored as a suffix in the name of the file, can be registered in the name of different applications. These applications can handle those file types.

The LairOS Kernel uses the OLP to open these apps with the data about selected files that the app should respond about. The kernel uses a stored key value pair that is created during the installation of the app, in which the app IDs point to what file extensions they can handle.
...
On LairOS Settings, you can change these default opener applications, only for registered file types.&#x20;
...
To finish off your change, test LairOS by trying to open file of that type.
