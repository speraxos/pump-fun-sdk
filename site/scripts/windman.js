...
sessionSettings.keepvisible = keepVisibleSetting;
    ...
    if (logData.wsnappingSetting) {
        ...
        const isNearTop = logData.cursorY < VHInPixels;
        const isNearBottom = (logData.viewportHeight - logData.cursorY) < VHInPixels;
        const isNearLeft = logData.cursorX < VWInPixels;
        const isNearRight = (logData.viewportWidth - logData.cursorX) < VWInPixels;

        // Corner snapping (quadrants)
        if (isNearTop && isNearLeft) {
            // Top-left quadrant
            snapWindowToQuadrant(x, winuid, 'top-left');
        } else if (isNearTop && isNearRight) {
            // Top-right quadrant
            snapWindowToQuadrant(x, winuid, 'top-right');
        } else if (isNearBottom && isNearLeft) {
            // Bottom-left quadrant
            snapWindowToQuadrant(x, winuid, 'bottom-left');
        } else if (isNearBottom && isNearRight) {
            // Bottom-right quadrant
            snapWindowToQuadrant(x, winuid, 'bottom-right');
        } else if (isNearTop) {
            // Top edge - maximize
            maximizeWindow(winuid);
        } else if (isNearLeft) {
            // Left half
            snapWindowToHalf(x, winuid, 'left');
        } else if (isNearRight) {
            // Right half
            snapWindowToHalf(x, winuid, 'right');
        }
    }
...
function snapWindowToHalf(x, winuid, side) {
    suppressNudge = true;
    x.classList.add("snapping");
    if (side === 'left') {
        x.style.cssText = `left: 0; top: 0; width: calc(50% - 0px); height: calc(100% - ${navheight}px); right: auto;`;
    } else {
        x.style.cssText = `right: 0; top: 0; width: calc(50% - 0px); height: calc(100% - ${navheight}px); left: auto;`;
    }
    x.getElementsByClassName("flbtn")[0].innerHTML = "open_in_full";
...
function snapWindowToQuadrant(x, winuid, quadrant) {
    suppressNudge = true;
    x.classList.add("snapping");
    const halfHeight = `calc(50% - ${navheight / 2}px)`;
    
    switch(quadrant) {
        case 'top-left':
            x.style.cssText = `left: 0; top: 0; width: 50%; height: ${halfHeight}; right: auto; bottom: auto;`;
            break;
        case 'top-right':
            x.style.cssText = `right: 0; top: 0; width: 50%; height: ${halfHeight}; left: auto; bottom: auto;`;
            break;
        case 'bottom-left':
            x.style.cssText = `left: 0; bottom: ${navheight}px; width: 50%; height: ${halfHeight}; right: auto; top: auto;`;
            break;
        case 'bottom-right':
            x.style.cssText = `right: 0; bottom: ${navheight}px; width: 50%; height: ${halfHeight}; left: auto; top: auto;`;
            break;
    }
    x.getElementsByClassName("flbtn")[0].innerHTML = "open_in_full";
    winds[winuid]["visualState"] = "snapped";
...
function dragElement(elmnt) {
    ...
    if (gid(elmnt.id + "header")) {
        ...
        // Add touch support
        gid(elmnt.id + "header").ontouchstart = dragTouchStart;
    }
    ...
    function dragTouchStart(e) {
        if (isMobile) return; // On mobile, windows are fullscreen - no dragging needed
        if (isInsideIbtnsSide(e.target)) return;
        
        const pos = getEventPos(e);
        holdStart = Date.now();
        grabOffsetX = pos.x - elmnt.getBoundingClientRect().left;
        grabOffsetY = pos.y - elmnt.getBoundingClientRect().top;
        elmnt.style.position = 'absolute';
    ...
    function elementTouchDrag(e) {
        ...
        let targetLeft = pos.x - grabOffsetX;
        let targetTop = pos.y - grabOffsetY;
