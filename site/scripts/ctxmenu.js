...
async function getMenuItems(target) {
	...
	if (target.id === 'desktop' || target.closest('#desktop')) {
		// Check if right-clicking on an icon on the desktop
		const clickedIcon = target.closest('.app-shortcut');
		if (clickedIcon && target.closest('#desktop')) {
			const itemuid = clickedIcon.getAttribute('unid');
			const selectedIcons = document.querySelectorAll('#desktop .app-shortcut.selected');
			const multipleSelected = selectedIcons.length > 1;
			
			return [
				{ icon: 'open_in_new', label: 'Open', action: () => {
					if (multipleSelected) {
						selectedIcons.forEach(icon => {
							const uid = icon.getAttribute('unid');
							if (uid) openfile(uid);
						});
					} else if (itemuid) {
						openfile(itemuid);
				...
				{ icon: 'edit', label: 'Rename', action: async () => {
					if (itemuid) {
						let file = await getFileById(itemuid);
						file.fileName = await ask('New Name:', file.fileName);
						await updateFile(false, itemuid, file);
				...
				{ icon: 'delete', label: multipleSelected ? `Delete (${selectedIcons.length})` : 'Delete', action: () => {
					if (multipleSelected) {
						selectedIcons.forEach(icon => {
							const uid = icon.getAttribute('unid');
							if (uid) remfile(uid);
				...
				{ icon: 'info', label: 'Properties', action: () => {
					if (itemuid) useHandler('settings_manager', { data: 'appsets', id: itemuid });
				}},
			];
		}
		...
		return [
			...
			{ icon: 'display_settings', label: 'Display Settings', action: () => openapp('settings') },
		];
	}
...
async function showContextMenu(event, isTouch = false) {
	...
	menuItems.slice(0, 6).forEach(item => {
		...
		menuItem.style.padding = '5px 10px';
		menuItem.style.cursor = 'pointer';
		// Use touchend for mobile to prevent ghost clicks
		const clickEvent = isTouch ? 'touchend' : 'click';
		menuItem.addEventListener(clickEvent, (e) => {
			e.preventDefault();
			item.action();
			contextMenu.style.display = 'none';
		});
	...
	const dialog = event.target.closest('dialog');
	const dialogRect = dialog ? dialog.getBoundingClientRect() : null;
	const cursorX = dialogRect ? clientX - dialogRect.left : clientX;
	const cursorY = dialogRect ? clientY - dialogRect.top : clientY;
