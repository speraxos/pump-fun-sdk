...
async function getMenuItems(target) {
	...
	if (target.id === 'desktop' || target.id === 'widget-layer') {
		return [
			{ icon: 'widgets', label: 'Add widget', action: () => openWidgetPicker() },
			...
			{ icon: 'refresh', label: 'Refresh homescreen', action: () => pumprefresh() },
